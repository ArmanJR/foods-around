...
class CrwController < ApplicationController
	def index
		...
		links << link
		break if link == "https://www.reyhoon.com/restaurants/boof-tirajeh"
		end
		@li = links
		...
		doc = Nokogiri::HTML(open(url))
			...
			#image = food.at_css('div').at_css('img').attr('scr')
			...
			#image = food.xpath('//*[@itemprop="image"]').first.attr('src')
			#image = food.at_css('._3ueAR').at_css('img').attr('src')
			
			#if(title.include?("آب")||title.include?("دوغ")||title.include?("نوشابه")||title.include?("ایستک")||title.include?("دلستر")||title.include?("ماست")||title.include?("فانتا")||title.include?("کوکا")||title.include?("‍سالاد")||title.include?("نان")||title.include?("ترشی")||title.include?("قوطی"))
			#	hash[:category] = "side"
			#end
			@foods << hash
			end
